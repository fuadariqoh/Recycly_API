...
module.exports = {
  register: (req, res) => {
    ...
    db.query(sql, (err, result) => {
      ...
      db.query(sql, (err6,result6)=>{
        if (err6) return res.status(500).send({err, message: 'error get duplicate username'});
        if (result6.length) {
  ...
  login:(req,res)=>{
    ...
    db.query(sql,(err,result)=>{ 
        if(err) return res.status(500).send({err, message:'error get username'})
        if(result.length){ //untuk ambil data cart
            sql= `SELECT * FROM users 
                  WHERE username='${username}' AND password='${encrypt(password)}'`
            db.query(sql,(err1,result1)=>{
              if(err1) return res.status(500).send({err1, message:'error get user'})
              if(result1.length){
                const token=createJWTToken({id:result1[0].id,username:result1[0].username})
                res.status(200).send({...result1[0],status:true,token:token})
              }
              else{
                return res.status(200).send({status:false,incorrectPassword:true})
              }
            })
        }else{
  ...
  sendEmailPassword: (req, res) => {
    ...
    db.query(sql,(err,result)=>{
      if(err) return res.status(500).send({err, message:'error get username'})
      if(result.length){ //untuk cari emailnya
        const token = createJWTToken({
          userid:result[0].id,
  ...
  ResetPassword: (req,res) =>{
    ...
    var sql=` UPDATE users SET ? 
              WHERE id=${userid}`;
    db.query(sql, obj, (err,result)=>{
      if(err) return res.status(500).send({err, message:'error update password'})
      sql = ` SELECT * FROM users 
              WHERE id=${userid}`;
      db.query(sql, (err1, result1) => {
